...
export default function PsikotesHistory() {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      if (isFirebaseConfigured && auth.currentUser) {
        try {
          const querySnapshot = await getDocs(collection(db, "users", auth.currentUser.uid, "psikotes_history"));
          const history = [];
          ...
          console.error("Gagal memuat riwayat dari Firestore:", e);
        }
      } else {
        // Offline fallback: load from localStorage
        const storedHistory = localStorage.getItem("psikotes_history");
        if (storedHistory) {
    ...
    // Use auth listener to ensure currentUser is populated on mount
  ...
  const clearHistory = async () => {
    if (confirm("Apakah Anda yakin ingin menghapus semua riwayat tes psikotes?")) {
      setLoading(true);
      if (isFirebaseConfigured && auth.currentUser) {
        try {
          const querySnapshot = await getDocs(collection(db, "users", auth.currentUser.uid, "psikotes_history"));
          ...
          querySnapshot.forEach((document) => {
            deletePromises.push(
              deleteDoc(doc(db, "users", auth.currentUser.uid, "psikotes_history", document.id))
            );
          });
        ...
        localStorage.removeItem("psikotes_history");
      }
      setHistoryList([]);
      setLoading(false);
    }
  ...
  return (
    <>
      <Head>
        <title>Riwayat Psikotes - BelajarMengabdi</title>
      </Head>
        ...
        <div className="card-frame">
          <div>
            ...
            <div className="header-brand" style={{ marginBottom: "24px" }}>
              ...
              <p className="brand-subtitle">Hasil Evaluasi Psikotes</p>
            </div>
            ...
            {loading ? (
              ...
              <div style={{
                ...
                <p style={{ fontSize: "12px", opacity: 0.9 }}>Kerjakan simulasi tes terlebih dahulu untuk melihat hasil evaluasi di sini.</p>
              </div>
            ) : (
                  ...
                  return (
                    <div key={item.id} className="history-item">
                      ...
                      <div className="history-score" style={{ color: isHigh ? "#ffffff" : "var(--bg-input)" }}>
                        {item.score}
                        {(item.category === "Kepribadian" || item.category === "Sikap Kerja") ? "%" : ""}
                      </div>
                    </div>
                  );
                })}
          ...
          <div className="bottom-action-btn">
            ...
            <button onClick={() => router.push("/psikotes")} className="btn btn-secondary">
              Kembali Ke Menu
            </button>
          </div>
        </div>
